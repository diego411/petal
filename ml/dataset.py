...
def create_spectrogram_images() -> Path:
    ...
    spectrogram_path = BASE_DATA_PATH / 'pre-labeled/spectrograms'
    ...
    for emotion in all_emotions:
        ...
        path = BASE_DATA_PATH / f'pre-labeled/audio/{emotion}'
        walker_wav = sorted(str(p) for p in Path(path).glob(f'*.wav'))
        walker_mp3 = sorted(str(p) for p in Path(path).glob(f'*.mp3'))
...
def get_image_dataset() -> ImageFolder:
    spectrogram_path: Path = create_spectrogram_images()
