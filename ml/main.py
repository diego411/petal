...
from ml.dataset import get_data_loaders
...
def train_model(model, train_dataloader, validation_dataloader):
    epochs = 20
    learning_rate = 0.01  # best_trial.config["lr"]
    ...
    for t in range(epochs):
        ...
        (_, accuracy) = test(validation_dataloader, model, cost, verbose=1)
        ...
        torch.save(model.state_dict(), f'{directory}/{epochs}_{date}_{accuracy}.pth')
